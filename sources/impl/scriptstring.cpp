...
//	r = engine->RegisterObjectBehaviour("string", asBEHAVE_FREE, "void f(string &in)", asFUNCTION(StringFree), asCALL_CDECL); assert( r >= 0 );
	...
	// Note: We must register the string factory after tbasic behaviours,
	// otherwise the library will not allow the use of object handles for this type
//	r = engine->RegisterStringFactory("string@", asFUNCTION(StringFactory)); assert( r >= 0 );
...
//	r = engine->RegisterObjectBehaviour("string", asBEHAVE_FREE, "void f(string &in)", asFUNCTION(StringFree), asCALL_CDECL); assert( r >= 0 );
	...
	// otherwise the library will not allow the use of object handles for this type
//	r = engine->RegisterStringFactory("string@", asFUNCTION(StringFactory_Generic)); assert( r >= 0 );
